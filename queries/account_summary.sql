...
SELECT
  ...
  + (SELECT COALESCE(SUM(ts.outflow - ts.inflow), 0) FROM transaction_category AS ts JOIN "transaction" AS t ON t.id = ts."transaction" WHERE ts.other_account = @account_id AND t.reconciled) AS reconciled_balance;
...
SELECT
  ...
  + (SELECT COALESCE(SUM(ts.outflow - ts.inflow), 0) FROM transaction_category AS ts JOIN "transaction" AS t ON t.id = ts."transaction" WHERE ts.other_account = a.id AND t.reconciled) AS reconciled_balance
FROM account AS a
WHERE a.budget = ?
ORDER BY a.name;
