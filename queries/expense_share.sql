...
SELECT
  b.id,
  @expense_share_id,
  (SELECT default_name FROM expense_share WHERE id = @expense_share_id) AS name
FROM
  budget AS b
WHERE
  b.login_id = @login_id
...
JOIN budget_expense_share AS bes ON e.id = bes.expense_share_id
JOIN budget as b ON b.id = bes.budget_id
WHERE b.id = @budget_id AND b.login_id = @login_id AND e.id = @expense_share_id
RETURNING
  *;

-- name: GetExpenseShareByCode :one
SELECT sqlc.embed(e), sqlc.embed(esc)
FROM expense_share AS e
JOIN expense_share_code AS esc ON esc.expense_share_id = e.id AND esc.code = @code
