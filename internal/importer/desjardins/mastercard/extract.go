...
// word are ~0pt, while gaps between columns are larger.
const gapThreshold = 1.0

// extract reads a PDF from data and returns its text as a structured document.
//
// AccesD statements position text with cm/Tm matrices that GetTextByRow
// ignores, so this uses Content().Text which applies the full text matrix.
