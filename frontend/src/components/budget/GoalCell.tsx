...
export default function GoalCell({ goal, onOpen }: GoalCellProps) {
  if (!goal) {
    return (
      <button
        ...
        title="Set goal"
        aria-label="Set goal"
        className="relative z-10 ml-auto w-28 cursor-pointer rounded px-1.5 py-0.5 text-right text-sm tabular-nums text-slate-600 transition hover:bg-slate-800/60 hover:text-slate-300"
      >
        Set goal
      </button>
    )
  ...
  return (
    <button
      ...
      className={`relative z-10 ml-auto flex w-28 cursor-pointer items-center justify-end gap-1 rounded px-1.5 py-0.5 text-sm tabular-nums transition ${
        offTrack
